...
Первый раз здесь? Четыре шага — и всё работает. Регистрироваться заранее не нужно.

1. **Скачайте** `StabiLink-Desktop-vX.Y.Z-win-x64.zip` из [последнего релиза](https://github.com/sany86russ/StabiLink-Desktop/releases/latest) и [сверьте SHA-256](#проверка-подлинности-сборки).
2. **Распакуйте** архив в отдельную папку — запускать прямо из ZIP нельзя.
3. **Запустите** `StabiLink.Desktop.exe` и нажмите «Да» на запрос Windows: права администратора нужны для работы с сетью.
4. **Войдите** на вкладке «Email код»: введите почту, получите код в письме. Аккаунт создастся сам, пароль придумывать не нужно.

Дальше выберите стратегию **Phantom** и нажмите большую кнопку включения. Не помогло — запустите «Автоподбор».

| Не получается? | Куда идти |
|---|---|
| Подробно про установку и вход | [Первый запуск и авторизация](docs/FIRST_START.md) |
| Подключить iPhone или Android | [Karing: пошагово со скриншотами](docs/KARING.md) |
| Остались вопросы | [FAQ](docs/FAQ.md) · [@stabilink_bot](https://t.me/stabilink_bot) |
...
- [Karing: подключение телефона со скриншотами](docs/KARING.md)
...
> [!IMPORTANT]
> Регистрироваться заранее не нужно. Во вкладке «Email код» введите почту и подтвердите код — если аккаунта ещё нет, StabiLink создаст его автоматически. Аккаунт на [apps.stabilink.ru](https://apps.stabilink.ru) заводите заранее, только если хотите сразу задать пароль или оплатить PRO.

1. Скачайте [последний Release](https://github.com/sany86russ/StabiLink-Desktop/releases/latest).
2. Сверьте SHA-256 и полностью распакуйте ZIP.
3. Запустите `StabiLink.Desktop.exe` от имени администратора.
4. Войдите на вкладке «Email код»: введите почту и подтвердите код из письма.
5. Оставьте Phantom и Game Filter включёнными.
...
Рекомендуемый вариант для Discord, голосовой связи и игровых соединений. Game Filter находится в настройках и включён по умолчанию.
...
Предварительная регистрация **не обязательна**. Во вкладке «Email код» введите действующую почту и запросите письмо: если аккаунта ещё нет, StabiLink создаст его автоматически. Входить так можно и дальше — вводить email и получать новый одноразовый код без пароля.

Создать аккаунт заранее на [apps.stabilink.ru](https://apps.stabilink.ru) стоит, если хотите сразу задать пароль или оплатить PRO. Тот же аккаунт открывается и в Telegram: [@stabilink_bot → Личный Кабинет](https://t.me/stabilink_bot/staboffice).
...
|---|---|---|
| **Email-код** | **Первый вход, аккаунта ещё нет** | Ввести почту, запросить письмо и подтвердить код; аккаунт создастся автоматически, пароль не нужен |
| **Email + пароль** | Аккаунт уже есть и пароль помните | Ввести email и пароль во вкладке «Пароль» |
| **QR-код** | Рядом есть телефон | Навести камеру телефона на QR и сравнить эмодзи. Телефона нет — кнопка «🖥 Открыть в браузере на этом ПК» |
| **Код устройства / TG-код** | Удобнее перепечатать короткий код | Получить код в кабинете и ввести во вкладке «TG код» |

Подробная таблица выбора и разбор каждой вкладки: [Первый запуск и авторизация](docs/FIRST_START.md).
...
1. Откройте [apps.stabilink.ru/devices](https://apps.stabilink.ru/devices) или кабинет в Telegram: [@stabilink_bot → Личный Кабинет](https://t.me/stabilink_bot/staboffice).
...
- **Karing:** «Добавить профиль» → «Импорт из буфера обмена» → галочка «✓» → включите VPN.
- **Happ:** откройте персональную ссылку кнопкой из кабинета либо импортируйте её вручную → включите профиль.
- **iOS:** разрешите вставку из буфера, затем подтвердите добавление VPN-конфигурации через Face ID/Touch ID/код устройства.
...
📱 **Пошагово со скриншотами каждого экрана: [Karing: подключение телефона](docs/KARING.md).**

Как устроены слоты, лимиты и безопасность ссылки: [Sub URL для Karing и Happ](docs/SUB_URL.md).

Интерактивный мастер с выбором Android/iPhone: [apps.stabilink.ru/setup](https://apps.stabilink.ru/setup).
...
| Поддержка | [@stabilink_bot](https://t.me/stabilink_bot) | Вопросы по аккаунту и оплате |
...
| Ошибки Desktop | [GitHub Issues](https://github.com/sany86russ/StabiLink-Desktop/issues/new/choose) | Воспроизводимые баги |

Личный кабинет в браузере и Mini-App в Telegram — это одно и то же место: общий аккаунт, подписка и список устройств. Telegram не обязателен.

> [!WARNING]
> Никогда не публикуйте токены, QR-коды, ссылки Sub URL, ключи и необработанные журналы — ни в Issues, ни в чатах. См. [Приватность и журналы](docs/PRIVACY_AND_LOGS.md).
